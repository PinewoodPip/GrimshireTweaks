...
- The crafting table can now use ingredients from chests on your farm (no need to have everything in your player inventory)
    ...
    - If an ingredient from the recipe is met by items in chests, it'll be highlighted in green to indicate so
...
- If the cursor is over the toolbar, clicking a slot will no longer also interact with an object in the world
...
- Added an option to skip the splash screen, mostly useful to speed up testing while modding
